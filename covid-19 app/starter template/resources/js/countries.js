...
// Select serch country Elements
const search_country_elements = document.querySelector(".search-country");
const country_list_elements = document.querySelector(".country-list");
...
function createCountryList() {
    ...
    country_list.forEach((country,index) => {
        if (index % Math.ceil(num_countries/num_of_ul_list) == 0){
            ul_list_id = `list-${i}`;
            country_list_elements.innerHTML+= `<ul id='${ul_list_id}'></ul> `;
            i++;
        }
...
change_country_btn.addEventListener('click', function() {
    input.value = "";
    resetCountryList();
    search_country_elements.classList.toggle("hide");
    search_country_elements.classList.add("fadeIn");
});
...
country_list_elements.addEventListener('click', function() {
    search_country_elements.classList.toggle("hide");
});
...
// Reset Country List(Show all the country)
